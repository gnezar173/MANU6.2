...
1. New Tables
- `companies`: Parent organizations that own one or more factories.
- `factories`: Individual factory sites, scoped to a company. Demo factory seeded.
- `departments`: Departments within a factory (production, quality, maintenance, etc.).
- `platform_users`: Users associated with a factory (managers, engineers, etc.).
- `subscription_plans`: SaaS pricing tiers (Starter, Professional, Enterprise).
- `data_quality_scores`: Per-factory data quality metrics.

2. Security
- Enable RLS on all new tables.
- Single-tenant demo mode: allow anon + authenticated CRUD.
- subscription_plans is SELECT-only (read-only reference data).

3. Important Notes
- The existing demo factory is seeded as the default factory with is_demo = true.
- Subscription plans are seeded with 3 tiers: Starter, Professional, Enterprise.
- All inserts use IF NOT EXISTS / ON CONFLICT guards for idempotency.
*/
