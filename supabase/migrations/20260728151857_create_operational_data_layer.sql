...
the hardcoded demo arrays. All tables are scoped to a factory via factory_id.

1. New Tables (all factory-scoped with composite primary keys)
- `production_lines`: Production lines within a factory
- `products`: Products manufactured (size, capacity, cycle time, cost, price)
- `machines`: Machines with OEE metrics, maintenance schedules
- `process_stages`: Ordered manufacturing process steps
- `raw_materials`: Raw material inventory with supplier info
- `shift_data`: Per-shift production records (planned vs actual, defects, energy)
- `defect_records`: Quality defect tracking
- `downtime_events`: Machine downtime tracking
- `energy_records`: Energy consumption by source (diesel, solar)
...
3. Demo Data
- Seeds demo operational data for the demo factory (production lines, products,
  machines, process stages, raw materials, defect records, downtime events).
- Generates 14 days of shift_data and energy_records via PL/pgSQL loop.
- All inserts are idempotent (guarded by NOT EXISTS checks).
*/
